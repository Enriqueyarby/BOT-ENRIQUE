...
const start = async (tobz = new Client()) => {
    ...
    console.log(color(figlet.textSync('BOT-ENRIQUE', {
        font: 'Ghost',
        horizontalLayout: 'default'
    })))
    console.log(color(figlet.textSync('----------------', {
    ...
    tobz.onMessage((async (message) => {
        ...
        tobz.getAmountOfLoadedMessages() // Cut message Cache if cache more than 3K
            .then((msg) => {
                if (msg >= 3000) {
                    console.log('[CLIENT]', color(`Alcance de mensaje cargado ${msg}, borrar la caché de mensajes...`, 'purple'))
                    tobz.cutMsgCache()
                }
            })
    ...
    tobz.onAddedToGroup(async (chat) => {
        if (isWhite(chat.id)) return tobz.sendText(chat.id, 'Hola, soy Enrique, escribe #help para ver mi lista de comandos ...')
        if (mtcState === false) {
            const groups = await tobz.getAllGroups()
            // BOT group count less than
            if (groups.length > groupLimit) {
                await tobz.sendText(chat.id, 'Lo sentimos, el límite de grupos que Enrique puede acomodar está completo').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            } else {
                if (chat.groupMetadata.participants.length < memberLimit) {
                    await tobz.sendText(chat.id, `Lo siento, BOT sale si los miembros del grupo no superan ${memberLimit} orang`).then(async () => {
                        tobz.deleteChat(chat.id)
                        tobz.leaveGroup(chat.id)
                    })
                } else {
                    if (!chat.isReadOnly) tobz.sendText(chat.id, 'Hola, soy Enrique, escribe #help para ver mi lista de comandos ...')
                }
            }
        } else {
            await tobz.sendText(chat.id, 'Enrique está en mantenimiento, prueba otro día').then(async () => {
                tobz.deleteChat(chat.id)
                tobz.leaveGroup(chat.id)
            })
        }
    ...
    tobz.onIncomingCall((async (call) => {
        await tobz.sendText(call.peerJid, 'Lo siento, no puedo recibir llamadas.= block !. \n Si desea abrir un bloque, ¡chatee con el propietario!')
...
function nocache(module, cb = () => {}) {
    console.log('Module', `'${module}'`, 'ahora está siendo observado por cambios')
